    ...
    <div>
      ...
      >
        ...
        <b-form-select v-model="selectedFont">
          <option v-for="item in fonts">{{item}}</option>
        </b-form-select>
      ...
      >
        <b-container>
          <b-alert show variant="info">Period: {{formatDate(time[0])}} - {{formatDate(time[1])}}</b-alert>
          <br>
        ...
        >
          <b-progress-bar
            v-if="Number(usage) <= Number(maxUsage)*0.66"
            :value="Number(usage)"
            variant="success"
          ></b-progress-bar>
          <b-progress-bar
            v-if="Number(usage) > Number(maxUsage)*0.66 && Number(usage) < Number(maxUsage)"
            :value="Number(usage)"
            variant="warning"
          ></b-progress-bar>
...
export default {
  ...
  watch: {
    selectedTheme: function(currentValue) {
      this.changeTheme();
    },
    selectedFont: function(currentValue) {
      this.changeFont();
    },
    selectedLanguage: function(currentValue) {
      this.changeLang();
    }
  },
  computed: {
  ...
  methods: {
    ...
    genData() {
      ...
      let data = [];
      for (
        var m = moment(this.time[0]);
        m.isBefore(moment(this.time[1]) + 1);
        m.add(1, "days")
      ) {
        x += Math.abs(r());
    ...
    changeTheme() {
      ...
      document.querySelectorAll("*").forEach(function(node) {
        // console.log(typeof node.tagName)
        if (node.style != undefined ) {
          // console.log(node.getAttribute('header'))
